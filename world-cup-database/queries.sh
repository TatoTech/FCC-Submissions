...
echo "$($PSQL "SELECT DISTINCT teams.name FROM teams JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.round = 'Eighth-Final' AND games.year = 2014 ORDER BY teams.name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id  WHERE round = 'Eighth-Final' ORDER BY name ASC")"
